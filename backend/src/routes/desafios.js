...
const COLUMNAS = `d.id, d.mision_id, d.orden, d.tipo, d.enunciado, d.puntos,
...
desafiosDeMisionRouter.post('/', async (req, res) => {
  ...
  try {
    ...
    const { rows } = await pool.query(
      `WITH nuevo AS (
         INSERT INTO desafios (mision_id, objeto_id, tipo, enunciado, orden)
         SELECT $1, $2, $3, $4, COALESCE(MAX(orden), 0) + 1
         FROM desafios WHERE mision_id = $1
         RETURNING *
       )
       SELECT ${COLUMNAS} FROM nuevo d LEFT JOIN objetos o ON o.id = d.objeto_id`,
      [misionId, valores.objeto_id, valores.tipo, valores.enunciado]
    );
...
// PUT /desafios/:id — modifica el enunciado, el tipo y el objeto asociado.
...
desafiosRouter.put('/:id', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `WITH cambiado AS (
         UPDATE desafios
         SET enunciado = $2, tipo = $3, objeto_id = $4
         WHERE id = $1
      ...
      [req.params.id, valores.enunciado, valores.tipo, valores.objeto_id]
    );
